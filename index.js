...
localStorage.setItem("playerArray",JSON.stringify(player))
localStorage.setItem("teamDetails",JSON.stringify(Data))

let teamData = JSON.parse(localStorage.getItem("teamDetails"))
let playerData = JSON.parse(localStorage.getItem("playerArray"))

let teamdetails = document.getElementById("teamdetails")

for (let i = 0; i < teamData.length; i++) {
    console.log(teamdetails.innerHTML)
    teamdetails.innerHTML +=
        ` <div class="teamcard" onclick="getTeamDetails(${teamData[i].id})">
    <img class="teamimage"
        src=${teamData[i].teamIcon}
        alt="">
        <div>
    <p class="teamname">${teamData[i].teamFullName}</p>
    <p class="winnings">Won Count : ${teamData[i].WonCount}</p>
    </div>
</div>
`
}

let Allplayerdetail = document.getElementById("Allplayerdetail")


for (let i = 0; i < playerData.length; i++) {
    console.log(playerData[i])
    Allplayerdetail.innerHTML +=
        `<div class="card" id="1" onclick="getPlayerDetails(${playerData[i].id})">
    <div class="img">
        <img src="${playerData[i].playerImg}">
            </div>
            <div class="details">
                <h3>${playerData[i].playerName}</h3>
                <h4>${playerData[i].description}</h4>
                <h5>Auction Price : ${playerData[i].price}</h5>
                <h5>${playerData[i].playerTeam}</h5>
                </div>
                </div>`
}
...
function getTeamDetails(event) {
    let teamName;
    let filterTeamData;
    let filterTeamPlayer;
    let filter = teamData.filter((team) => team.id === event)
    filterTeamData = filter
    teamName = filter[0].teamFullName
    let filterPlayers = playerData.filter((player) => player.playerTeam === teamName)
    filterTeamPlayer = filterPlayers
    localStorage.setItem("teamDetails", JSON.stringify(filterTeamData))
    localStorage.setItem("teamPlayers", JSON.stringify(filterTeamPlayer))
}
